...
std::ostream& operator<<(std::ostream& out, const HazMat7k &rhs){
    out << "Report Number and Date: " << rhs.reportNumber + ' ' << rhs.reportReceivedDate << "   "
        << "Local Date and Time: " << rhs.localDate << ' ' << rhs.localTime << "   ";
}

//sad lil default constr.
LossOrDamage::LossOrDamage() {setIncidentCode('l');}

//LossOrDamage's convert constr simply passes the whole thing off to HazMat7k.
LossOrDamage::LossOrDamage(const HazMatData &d) : HazMat7k(d) {setIncidentCode('l');}

//sad lil default constr.
FatalityOrInjury::FatalityOrInjury() {setIncidentCode('f');}
