...
public:
    ...
    ~Node(){delete info;}//as this specialization dynamically allocates
};
...
Node<HazMat7k*>::Node(HazMat7k *ptr7k, Node<HazMat7k*> *n)
{
    ...
    try{ //alloc mem according to the type of report
        ...
        else cout << "something went wrong inside specialized Node constructor with parameters\n"
                  << "there was no match to the Incident Code, should be either f or l\n"
                  << "see Milestone 4, changes required to the loadData function\n";
    
        info = addMe;
        next = n;
    }
    catch (std::bad_alloc baErr){throw;}
    catch (...){throw string("unknown error in constructor of specialized Node class");}
}
